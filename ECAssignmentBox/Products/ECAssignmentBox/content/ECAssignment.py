# -*- coding: utf-8 -*-
# $Id: ECAssignment.py 1296 2009-09-27 18:28:47Z amelung $
#
...
ECAssignmentSchema = ECAssignmentSchema + Schema((
    ...
    FileField(
        ...
        searchable = True,
        widget = FileWidget(
            label = "Text field answer",
            label_msgid = "label_answer_text",
            description = "The submission for this assignment from the text field",
            description_msgid = "help_answer_text",
            i18n_domain = config.I18N_DOMAIN,
            macro = 'answer_widget',
        ),
    ),
...
class ECAssignment(BaseContent, HistoryAwareMixin):
    ...
    #security.declarePublic('get_data_text')
    def get_data_text(self):
        """ 
        Introduced to Display content of the field 'text' which has
        to be displayed differently then file.  
        
        It doesn't have to be checked what it is because it is clear
        that it comes out of the text box.
        """
    ...
    def getAsPlainText(self):
        ...
        t = self.getField('text')
        #source = ''

        mt = self.getContentType('file')
	
        content_file = str(f.get(self))
        content_text = str(t.get(self))

        if content_text and content_file:
            if not re.match('text/|application/(.+\+)?xml',mt):
                return content_text
    ...
    def getViewModeReadFieldNames(self):
        ...
        """
	text = StringIO(self.getField('text').get(self)).read()
        file = StringIO(self.getField('file').get(self)).read() 
	if (text != '') and (file != ''):
            return ['text', 'file', 'remarks', 'feedback', 'mark']
